...
### 微信：把 Jidan 接进微信，而不是冒充用户控制微信

腾讯已开源 `openclaw-weixin`，通过二维码授权把 Agent 作为微信里的对话通道接入。这是一条官方、可维护的入口，适合“用户在微信里给 Jidan 下任务，Jidan 回传进度与结果”。它并不授予 Jidan 代表用户给任意现有联系人发消息：发送依赖该 Bot 会话产生的目标 ID 与上下文令牌。因此它可作为 Jidan 的微信入口和回执通道，不能伪装成“控制第一个好友”的 API。[Tencent/openclaw-weixin](https://github.com/Tencent/openclaw-weixin)。
